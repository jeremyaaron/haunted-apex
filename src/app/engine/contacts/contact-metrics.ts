...
export function applyContactMetricDelta(
  ...
  return {
    ...updated,
    burned: contact.burned || shouldBurnContact(updated),
  };
}

function shouldBurnContact(contact: ContactState): boolean {
  return (
    contact.exposure >= 100 ||
    contact.volatility >= 100 ||
    (contact.trust <= 0 && contact.volatility >= 75)
  );
}
